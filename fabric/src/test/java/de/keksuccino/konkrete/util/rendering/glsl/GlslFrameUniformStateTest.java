...
class GlslFrameUniformStateTest {
    ...
    void writesOneFrameInputSnapshotConsistentlyAcrossBufferAndImagePasses() {
        ...
        List<String> frameScopedMembers = List.of("iResolution", "iTime", "iTimeDelta", "iFrameRate", "iMouse", "iDate", "iSampleRate", "iChannelTime", "iChannelResolution", "konkreteAreaSize", "konkreteScreenSize", "konkreteGuiScale", "konkreteMouse", "konkreteMouseDelta", "konkreteMouseButtons", "konkreteMouseClickCount", "konkreteMouseReleaseCount", "konkreteMouseScroll", "konkreteMouseScrollTotal", "konkreteKeyEvent", "konkreteKeyEventCount", "konkreteCharEvent", "konkreteCharEventCount", "konkreteDateParts", "konkreteTimeParts", "konkreteDayOfYear", "konkreteWeekOfYear", "konkreteUnixTimeSeconds", "konkreteUnixTimeMilliseconds", "konkretePartialTick", "konkreteGameDeltaTicks", "konkreteRealtimeDeltaTicks", "konkreteInWorld", "konkreteIsPaused", "konkreteOpacity", "konkreteVariableCount");
        for (String member : frameScopedMembers) {
            assertMemberBytesEqual(layout, member, bufferPass, imagePass);
        }
        assertAll(() -> assertIntVector(layout, bufferPass, "iFrame", 7), () -> assertIntVector(layout, imagePass, "iFrame", 42), () -> assertFloatVector(layout, bufferPass, "konkreteMouseDelta", 5.0F, -6.0F), () -> assertFloatVector(layout, imagePass, "konkreteMouseDelta", 5.0F, -6.0F), () -> assertFloatVector(layout, bufferPass, "konkreteMouseScroll", 1.25F, -2.5F), () -> assertFloatVector(layout, imagePass, "konkreteMouseScroll", 1.25F, -2.5F), () -> assertIntVector(layout, bufferPass, "konkreteKeyEvent", 65, 30, 2, 1), () -> assertIntVector(layout, imagePass, "konkreteKeyEvent", 65, 30, 2, 1));
    }
    ...
    void writesAllDynamicVariableFamiliesAndKeepsTheOriginalVariableCount() {
        ...
        assertAll(() -> assertFloatVector(layout, buffer, "konkreteVarFloat_score", 1.5F), () -> assertIntVector(layout, buffer, "konkreteVarInt_score", 2), () -> assertIntVector(layout, buffer, "konkreteVarBool_score", 1), () -> assertFloatVector(layout, buffer, "konkreteVarVec2_score", 3.0F, 4.0F), () -> assertFloatVector(layout, buffer, "konkreteVarVec3_score", 3.0F, 4.0F, 5.0F), () -> assertFloatVector(layout, buffer, "konkreteVarVec4_score", 3.0F, 4.0F, 5.0F, 6.0F), () -> assertIntVector(layout, buffer, "konkreteVarExists_score", 1), () -> assertIntVector(layout, buffer, "konkreteVariableCount", 9));
    }
    ...
    void repeatedPassWritesDoNotConsumeOrMutateFrameDeltas() {
        ...
        assertAll(() -> assertEquals(first, second), () -> assertFloatVector(layout, first, "konkreteMouseDelta", 5.0F, -6.0F), () -> assertFloatVector(layout, second, "konkreteMouseDelta", 5.0F, -6.0F), () -> assertFloatVector(layout, first, "konkreteMouseScroll", 1.25F, -2.5F), () -> assertFloatVector(layout, second, "konkreteMouseScroll", 1.25F, -2.5F));
    }

    private static GlslStd140Layout uniformLayout() {
        String source = """
                uniform float konkreteVarFloat_score;
                uniform int konkreteVarInt_score;
                uniform bool konkreteVarBool_score;
                uniform vec2 konkreteVarVec2_score;
                uniform vec3 konkreteVarVec3_score;
                uniform vec4 konkreteVarVec4_score;
                uniform int konkreteVarExists_score;
                void main() {
                    gl_FragColor = vec4(konkreteVarFloat_score + float(konkreteVarInt_score + konkreteVarBool_score + konkreteVarExists_score)) + konkreteVarVec4_score + vec4(konkreteVarVec2_score, konkreteVarVec3_score.x, 0.0);
                }
                """;
