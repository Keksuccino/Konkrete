...
public class HorizontalSwitcher extends Gui {
	...
	public void render(int x, int y) {
		...
		this.prev.yPosition = y;
		this.prev.drawButton(Minecraft.getMinecraft(), mouseX, mouseY);
		
		//Value background
		drawRect(x + 25, y, x + 25 + this.width, y + 20, this.valuebackcolor.getRGB());
		
		//Selected value
		drawCenteredString(Minecraft.getMinecraft().fontRendererObj, sel, x + 25 + (this.width/2), y + 5, this.valuecolor.getRGB());
		
		this.next.xPosition = x + 25 + this.width + 5;
		this.next.yPosition = y;
		this.next.drawButton(Minecraft.getMinecraft(), mouseX, mouseY);
	}
