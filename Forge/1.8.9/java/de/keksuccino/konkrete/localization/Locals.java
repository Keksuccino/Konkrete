...
public class Locals {
	...
	public static void copyLocalsFileToDir(ResourceLocation file, String language, String saveDirWithoutFilename) {
		...
		try {
			try {
				br = new BufferedReader(new InputStreamReader(Minecraft.getMinecraft().getResourceManager().getResource(file).getInputStream(), StandardCharsets.UTF_8));
	...
	public static String localize(String key, String... dynamicValues) {
		String playerLang = Minecraft.getMinecraft().gameSettings.language.toLowerCase();
