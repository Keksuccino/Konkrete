...
public class AdvancedButton extends Button {
	...
	public void renderButton(int mouseX, int mouseY, float partialTicks) {
		if (this.visible) {
			...
			if (this.hasColorBackground()) {
				Color border;
				if (!isHovered) {
					fill(this.x, this.y, this.x + this.width, this.y + this.height, this.idleColor.getRGB() | MathHelper.ceil(this.alpha * 255.0F) << 24);
					border = this.idleBorderColor;
				} else {
					if (this.active) {
						fill(this.x, this.y, this.x + this.width, this.y + this.height, this.hoveredColor.getRGB() | MathHelper.ceil(this.alpha * 255.0F) << 24);
	...
	protected void renderLabel() {
		...
		if (this.labelShadow) {
			font.drawStringWithShadow(getMessage(), pX, pY, getFGColor() | MathHelper.ceil(this.alpha * 255.0F) << 24);
		} else {
			font.drawString(getMessage(), pX, pY, getFGColor() | MathHelper.ceil(this.alpha * 255.0F) << 24);
		}
