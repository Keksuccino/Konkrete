...
public class ConfigScreen extends Screen {
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		...
		fill(0, 0, this.width, this.height, SCREEN_BACKGROUND_COLOR.getRGB());
		
		this.configList.render();
		
		//Draw header
		fill(0, 0, this.width, 50, HEADER_FOOTER_COLOR.getRGB());
		...
		//Draw footer
		fill(0, this.height - 50, this.width, this.height, HEADER_FOOTER_COLOR.getRGB());
		
		this.doneBtn.setX((this.width / 2) - (this.doneBtn.getWidth() / 2));
		this.doneBtn.setY(this.height - 35);
		this.doneBtn.render(mouseX, mouseY, partialTicks);
	...
	protected static void renderDescriptionBackground(int x, int y, int width, int height) {
		fill(x, y, x + width, y + height, new Color(26, 26, 26, 250).getRGB());
	}
	...
	protected static abstract class ConfigScrollAreaEntry extends ScrollAreaEntry {
		...
		public void renderEntry() {
			...
			fill(this.x, this.y, this.x + this.getWidth(), this.y + this.getHeight(), ENTRY_BACKGROUND_COLOR.getRGB());
	...
	protected static class StringConfigScrollAreaEntry extends ConfigScrollAreaEntry {
		...
		public void render() {
			...
			input.setY(this.y + 3);
			input.render(MouseInput.getMouseX(), MouseInput.getMouseY(), Minecraft.getInstance().getRenderPartialTicks());
	...
	protected static class BooleanConfigScrollAreaEntry extends ConfigScrollAreaEntry {
		...
		public BooleanConfigScrollAreaEntry(ScrollArea parent, ConfigEntry configEntry) {
			...
			toggleBtn = new AdvancedButton(0, 0, 102, 20, "", true, (press) -> {
				state = !state;
				if (state) {
					toggleBtn.setMessage("§a" + Locals.localize("configscreen.boolean.enabled"));
				} else {
					toggleBtn.setMessage("§c" + Locals.localize("configscreen.boolean.disabled"));
				}
			});
			if (state) {
				toggleBtn.setMessage("§a" + Locals.localize("configscreen.boolean.enabled"));
		...
		public void render() {
			...
			toggleBtn.render(MouseInput.getMouseX(), MouseInput.getMouseY(), Minecraft.getInstance().getRenderPartialTicks());
