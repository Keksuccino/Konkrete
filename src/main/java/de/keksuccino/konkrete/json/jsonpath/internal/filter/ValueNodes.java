        ...
        public ValueNode evaluate(Predicate.PredicateContext ctx) {
            if (isExistsCheck()) {
                ...
                try {
                    ...
                    if (res instanceof Number) return createNumberNode(res.toString());
                    else if (res instanceof String) return createStringNode(res.toString(), false);
                    else if (res instanceof Boolean) return createBooleanNode(res.toString());
                    else if (res instanceof OffsetDateTime) return createOffsetDateTimeNode(res.toString()); //workaround for issue: https://github.com/json-path/JsonPath/issues/613
