...
/** Provides {@code AfmaBlockInterPayloadHelper} transformations and validation used by the AFMA codec. */
public final class AfmaBlockInterPayloadHelper {
    ...
    /** Writes the payload to the AFMA codec output. */
    @NotNull
    ...
    /** Writes the tile operation to the AFMA codec output. */
    ...
    /** Iterates over the payload in the AFMA codec data. */
    ...
    /** Calculates the tile count value for the AFMA codec. */
    ...
    /** Calculates the tile dimension value for the AFMA codec. */
    ...
    /** Calculates the expected raw tile bytes value for the AFMA codec. */
    ...
    /** Validates the channels before AFMA codec processing. */
    ...
    /** Validates the length before AFMA codec processing. */
    ...
    /** Validates the expected length before AFMA codec processing. */
    ...
    /** Validates the unsigned short length before AFMA codec processing. */
    ...
    /** Selects the {@code TileMode} representation or behavior used by the AFMA codec. */
    public enum TileMode {
        /** Selects the {@code SKIP} serialized AFMA representation. */
        SKIP(0),
        /** Selects the {@code COPY} serialized AFMA representation. */
        COPY(1),
        /** Selects the {@code COPY_DENSE} serialized AFMA representation. */
        COPY_DENSE(2),
        /** Selects the {@code COPY_SPARSE} serialized AFMA representation. */
        COPY_SPARSE(3),
        /** Selects the {@code RAW} serialized AFMA representation. */
        RAW(4);
    ...
    /** Carries {@code TileOperation} data between validated stages of the AFMA codec. */
    public record TileOperation(@NotNull TileMode mode, int dx, int dy, int channels, int changedPixelCount,
        ...
        /** Validates constructor components before storing this {@code TileOperation}. */
    ...
    /** Consumes {@code TileConsumer} values emitted by the AFMA codec. */
    @FunctionalInterface
    public interface TileConsumer {
        /** Consumes one decoded block-inter tile and its payload slices. */
    ...
    /** Reads {@code PayloadReader} input for the AFMA codec. */
    protected static final class PayloadReader {
        ...
        /** Initializes a new {@code PayloadReader} for AFMA codec use. */
        ...
        /** Reads the int from the AFMA codec input. */
        ...
        /** Reads the unsigned byte from the AFMA codec input. */
        protected int readUnsignedByte() throws IOException {
            this.ensureAvailable(1);
            return this.payloadBytes[this.offset++] & 0xFF;
        }

        /** Reads the unsigned short from the AFMA codec input. */
        ...
        /** Reads the short from the AFMA codec input. */
        protected int readShort() throws IOException {
            return (short) this.readUnsignedShort();
        }

        /** Returns the current absolute offset in the payload byte array. */
        protected int position() {
            return this.offset;
        }

        /** Returns the number of unread bytes in this bounded payload view. */
        protected int remaining() {
            return this.endOffset - this.offset;
        }

        /** Skips resource state in the AFMA codec. */
        protected void skip(int length) throws IOException {
            this.ensureAvailable(length);
            this.offset += length;
        }

        /** Ensures the available is valid for the AFMA codec. */
