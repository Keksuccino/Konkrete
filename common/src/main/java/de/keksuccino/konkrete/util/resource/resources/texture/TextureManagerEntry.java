...
final class TextureManagerEntry<I extends AutoCloseable, T extends AutoCloseable> implements AutoCloseable {
    ...
    /**
     * Atomically closes an unregistered image or dispatches one texture-manager release for a registered texture.
     * Repeated and concurrent calls are harmless.
     */
    @Override
    public void close() {
        I image;
    ...
    private void releaseRegisteredTexture(@Nullable Identifier identifier, @Nullable T texture, boolean releaseRegisteredTexture) {
        if (releaseRegisteredTexture && (identifier != null)) {
            try {
                ...
                LOGGER.error("[KONKRETE] Failed to release dynamic texture-manager entry: {}", identifier, throwable);
            }
        }
