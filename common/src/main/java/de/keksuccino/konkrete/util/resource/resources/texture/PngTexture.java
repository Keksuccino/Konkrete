...
/** Loads a PNG from a local, web, or resource-pack source and owns dynamic textures it registers. */
@SuppressWarnings("unused")
public class PngTexture implements ITexture {

    private static final Logger LOGGER = LogManager.getLogger();

    /** Lazily creates the shared transparent PNG resource used as a renderable placeholder. */
    ...
    public static PngTexture location(@NotNull Identifier location, @Nullable PngTexture writeTo) {
        ...
        try {
            ...
            if (resource.isPresent()) {
                ...
                LOGGER.error("[KONKRETE] Failed to read texture from Identifier! Resource not present: " + location, new IOException());
            }
        } catch (Exception ex) {
            texture.loadingFailed = true;
    ...
    /** Initializes a new {@code PngTexture} for texture resource use. */
    ...
    public Identifier getResourceLocation() {
        ...
        if (this.textureEntry.canRegister()) {
            try {
                this.textureEntry.register(ResourceRuntime.identifier("dynamic/simple_texture_" + ResourceRuntime.nextResourceId()));
            } catch (Exception ex) {
                LOGGER.error("[KONKRETE] Failed to register PNG texture to Minecraft's TextureManager!", ex);
            }
        }
    ...
    /** Returns the native image, or {@code null} when it is not available. */
    @Nullable
