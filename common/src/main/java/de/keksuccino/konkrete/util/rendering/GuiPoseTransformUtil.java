...
final class GuiPoseTransformUtil {
    ...
    private static final float DEFAULT_SCALE_KONKRETE = 1.0F;
    ...
    static PoseTransform resolve(@Nonnull GuiGraphicsExtractor graphics) {
        ...
        float scaleX = axisLength_Konkrete(pose.m00(), pose.m01());
        float scaleY = axisLength_Konkrete(pose.m10(), pose.m11());
        float scale = resolveScale_Konkrete(scaleX, scaleY);
        float translationX = finiteOrDefault_Konkrete(pose.m20(), 0.0F);
        float translationY = finiteOrDefault_Konkrete(pose.m21(), 0.0F);
    ...
    private static float resolveScale_Konkrete(float scaleX, float scaleY) {
        if (scaleX == 0.0F && scaleY == 0.0F) {
            return DEFAULT_SCALE_KONKRETE;
        }
        if (scaleX == 0.0F) {
            return scaleY;
        }
    ...
    private static float axisLength_Konkrete(float x, float y) {
        float length = (float)Math.sqrt(x * x + y * y);
        return Float.isFinite(length) && length > 0.0F ? length : 0.0F;
    }

    private static float finiteOrDefault_Konkrete(float value, float fallback) {
