...
public final class AsyncLoadCoordinator<K> {
    ...
    private static final int DEFAULT_MAXIMUM_IN_FLIGHT = 256;
    private final Object lifecycleLock = new Object();
    private final Map<K, Claim<K>> inFlight = new HashMap<>();
    private final int maximumInFlight;
    private long generation;

    /** Creates a coordinator admitting at most 256 distinct keys at once. */
    public AsyncLoadCoordinator() {
        this(DEFAULT_MAXIMUM_IN_FLIGHT);
    }

    /** Creates a coordinator with an explicit positive bound on distinct admitted keys. */
    public AsyncLoadCoordinator(int maximumInFlight) {
        if (maximumInFlight <= 0) throw new IllegalArgumentException("maximumInFlight must be positive");
    ...
    public @Nullable Claim<K> tryClaim(@NotNull K key, long startedAt, @NotNull BooleanSupplier admissionCheck) {
        ...
        synchronized (this.lifecycleLock) {
            if (this.inFlight.containsKey(key) || this.inFlight.size() >= this.maximumInFlight || !admissionCheck.getAsBoolean()) return null;
    ...
    /** Returns whether the current generation has an admitted load for {@code key}. */
    ...
    /** Returns the generation incremented by each atomic {@link #reset(Runnable)}. */
    ...
    /** Identity token for one admitted key load; only its owning coordinator may consume it. */
    public static final class Claim<K> {
        ...
        /** Returns the cache key claimed for this load. */
        @NotNull
        public K key() {
            return this.key;
        }

        /** Returns the caller-supplied admission timestamp used for timeout decisions. */
        public long startedAt() {
            return this.startedAt;
        }

        /** Returns whether timeout, reset, abandonment, or pre-run cancellation invalidated this claim. */
