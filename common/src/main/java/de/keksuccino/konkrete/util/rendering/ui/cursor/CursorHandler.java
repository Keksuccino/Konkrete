package de.keksuccino.konkrete.util.rendering.ui.cursor;
...
/** Coordinates cursor lifecycle and event dispatch. */
@SuppressWarnings("unused")
public class CursorHandler {

    private static final Logger LOGGER = LogManager.getLogger();
    private static final CursorHandleLifecycle CURSOR_HANDLE_LIFECYCLE = new CursorHandleLifecycle(new MinecraftGlfwThreadExecutor(), new GlfwNativeOperations(), throwable -> LOGGER.error("[KONKRETE] Failed to release a GLFW cursor!", throwable));
    ...
    public static long getActiveCursor() {
        try {
            ...
            return GlfwCursorTracker.getActiveCursor(de.keksuccino.konkrete.util.window.WindowHandler.getWindowHandle());
    ...
    /** Idempotently enables custom-cursor registration before the first client tick. */
    public static void init() {
        if (initialized) return;
        initialized = true;
    }

    /** Registers a named cursor and retires any different cursor previously stored under that name. */
    public static void registerCustomCursor(@NotNull String uniqueCursorName, @NotNull CustomCursor cursor) {
        if (!initialized) throw new RuntimeException("[KONKRETE] CursorHandler accessed too early!");
        LOGGER.info("[KONKRETE] Registering GLFW custom cursor: NAME: " + uniqueCursorName + " | TEXTURE CONTEXT: " + cursor.textureName);
        CUSTOM_CURSORS.register(Objects.requireNonNull(uniqueCursorName), Objects.requireNonNull(cursor), CustomCursor::isUsable);
    }

    /** Removes and asynchronously destroys the cursor registered under a name. */
    ...
    /** Retires an owned cursor while preserving a newer replacement registered under the same name. */
    ...
    /** Returns a live named cursor, or {@code null} when absent or already retired. */
    @Nullable
    ...
    private static void setCursor(long cursor) {
        if (!initialized) throw new RuntimeException("[KONKRETE] CursorHandler accessed too early!");
        GLFW.glfwSetCursor(de.keksuccino.konkrete.util.window.WindowHandler.getWindowHandle(), cursor);
    }

    /**
     * Applies the cursor selected for the current client tick and resets the selection.
     * Loader integrations should invoke this once near the start of each client tick.
     */
    ...
    /** Releases all Konkrete-owned cursors while Minecraft's GLFW window is still alive. */
    public static void shutdown() {
        LOGGER.info("[KONKRETE] Releasing Konkrete-owned GLFW cursors during client shutdown..");
    ...
    private static void destroyNativeCursor(long cursor) {
        ...
        for (long window : GlfwCursorTracker.getWindowsUsingCursor(cursor)) {
            try {
                ...
                LOGGER.error("[KONKRETE] Failed to switch a window away from a GLFW cursor before destroying it!", throwable);
            }
        }
    ...
    /** Describes a lazily created GLFW cursor backed by a classpath image. */
    public static class CustomCursor {

        /** Native GLFW cursor handle. */
        public final long id_long;
        /** Horizontal pixel offset of the pointer hotspot. */
        public final int hotspotX;
        /** Vertical pixel offset of the pointer hotspot. */
        public final int hotspotY;
        private final CursorHandleLifecycle.Handle nativeHandle;
        /** Source image retained for caller inspection; cursor destruction does not close it. */
        @NotNull
        public final PngTexture texture;
        /** Diagnostic name of the source image. */
        @NotNull
        public final String textureName;

        /** Decodes a ready PNG resource and allocates a native cursor, returning {@code null} on failure. */
        ...
        public static CustomCursor create(@NotNull PngTexture texture, int hotspotX, int hotspotY, @NotNull String textureName) {
            ...
            try {
                ...
                LOGGER.error("[KONKRETE] Failed to create custom GLFW cursor from " + textureName, ex);
            }
            if (texResourceBuffer != null) {
                try {
                    MemoryUtil.memFree(texResourceBuffer);
                } catch (Exception ex) {
                    LOGGER.error("[KONKRETE] Failed to free a custom cursor's encoded image buffer", ex);
                }
            }
            if (stbBuffer != null) {
                try {
                    ...
                    LOGGER.error("[KONKRETE] Failed to free a custom cursor's decoded image buffer", ex);
                }
            }
        ...
        /** Tracks an already allocated native cursor together with its non-owned source image. */
    ...
    private static final class MinecraftGlfwThreadExecutor implements CursorHandleLifecycle.ThreadExecutor {
        ...
        /** Reports whether the caller already runs on the required thread. */
        @Override
        ...
        public void execute(@NotNull Runnable task) {
            ...
            if (minecraft != null) {
                ...
                LOGGER.error("[KONKRETE] Could not schedule GLFW cursor cleanup because the Minecraft client is unavailable!");
            }
        }
    ...
    private static final class CustomCursorRetirement implements CursorRegistry.Retirement<CustomCursor> {
        ...
        /** Marks retired for the next lifecycle phase. */
        @Override
    ...
    private static final class GlfwNativeOperations implements CursorHandleLifecycle.NativeOperations {
        ...
        /** Stops accepting new native cursor work before GLFW shutdown. */
        @Override
        ...
        /** Destroys the supplied GLFW cursor handle when it is still valid. */
        @Override
