package de.keksuccino.konkrete.util.resource.resources.audio.ogg;
...
/** Owns asynchronous Ogg/Vorbis decoding and its OpenAL playback source. */
...
public class OggAudio implements IAudio, ALAudio {
    ...
    public static OggAudio location(@NotNull Identifier location, @Nullable OggAudio writeTo) {
        ...
        try {
            ...
            LOGGER.error("[KONKRETE] Failed to read OGG audio from Identifier: " + location, ex);
        }
    ...
    public static OggAudio local(@NotNull File oggAudioFile, @Nullable OggAudio writeTo) {
        ...
        if (!oggAudioFile.isFile()) {
            audio.loadingFailed = true;
            LOGGER.error("[KONKRETE] Failed to read OGG audio from file! File not found: " + oggAudioFile.getPath());
            return audio;
        }
        ...
        try {
            ...
            audio.loadingFailed = true;
            LOGGER.error("[KONKRETE] Failed to read OGG audio from file: " + oggAudioFile.getPath(), ex);
        }
    ...
    public static OggAudio web(@NotNull String oggAudioURL, @Nullable OggAudio writeTo) {
        ...
        if (!TextValidators.BASIC_URL_TEXT_VALIDATOR.get(oggAudioURL)) {
            audio.loadingFailed = true;
            LOGGER.error("[KONKRETE] Failed to read OGG audio from URL! Invalid URL: " + oggAudioURL);
            return audio;
        }
        ...
        KonkreteThreads.startDaemonThread(() -> {
            ...
            try {
                ...
                audio.loadingFailed = true;
                LOGGER.error("[KONKRETE] Failed to read OGG audio from URL: " + oggAudioURL, ex);
            }
        ...
        KonkreteThreads.startDaemonThread(() -> {
            ...
            try {
                ...
                try {
                    ...
                    LOGGER.warn("[KONKRETE] Failed to read OGG duration metadata, duration will be approximate: " + name, ex);
                }
                ...
                audio.loadingFailed = true;
                LOGGER.error("[KONKRETE] Failed to read OGG audio: " + name, ex);
            }
            CloseableUtils.closeQuietly(stream);
    ...
    /** Initializes a new {@code OggAudio} for audio resource use. */
    protected OggAudio() {
    ...
    private static void failBecauseOpenAlNotReady(@NotNull OggAudio audio, @NotNull String sourceName) {
        ...
        LOGGER.warn("[KONKRETE] Delaying OGG audio load because OpenAL is not ready yet or still reloading. It will retry automatically once ready again: " + sourceName);
    }
    ...
    private static void failBecauseOpenAlReload(@NotNull OggAudio audio, @NotNull String sourceName, @NotNull String reason) {
        ...
        audio.decoded = false;
        audio.retryWhenOpenAlReady = true;
        LOGGER.warn("[KONKRETE] Delaying OGG audio load because OpenAL is reloading (" + reason + "). It will retry automatically once ready again: " + sourceName);
    }
    ...
    public void setPlayTime(float playTime) {
        ...
        try {
            ...
            LOGGER.error("[KONKRETE] Failed to seek OGG audio preview play time!", ex);
        }
    }
    ...
    public int getALSource() {
        ...
        try {
            ...
            LOGGER.error("[KONKRETE] Failed to get AL source in OggAudio!", ex);
        }
        return 0;
    }
    ...
    private boolean configureNonPositionalSource(@NotNull String sourceName) {
        ...
        try {
            ...
            LOGGER.debug("[KONKRETE] OGG source configuration error details: " + sourceName, ex);
        }
        return false;
    }
    ...
    private boolean tryAttachDecodedBuffer(@NotNull ALAudioClip targetClip, @NotNull ALAudioBuffer decodedBuffer, @NotNull String sourceName) {
        ...
        try {
            ...
            LOGGER.debug("[KONKRETE] OGG buffer attach error details: " + sourceName, ex);
        }
        return false;
    }
    ...
    public void close() {
        ...
        try {
            ...
            LOGGER.error("[KONKRETE] Failed to close OGG audio clip!", ex);
        }
        this.clip = null;
        try {
            ...
            LOGGER.error("[KONKRETE] Failed to delete OGG audio buffer!", ex);
        }
