...
final class GlslFrameUniformState {
    ...
    @NotNull
    private Supplier<? extends Map<String, String>> variableProvider = Map::of;
    ...
    @NotNull
    private VariableSnapshot buildVariableValues() {
        Map<String, String> providedVariables = this.variableProvider.get();
        if (providedVariables == null || providedVariables.isEmpty()) return new VariableSnapshot(Map.of(), 0);
        List<Map.Entry<String, String>> variables = new ArrayList<>(providedVariables.entrySet());
        ...
        for (Map.Entry<String, String> variable : variables) {
            String suffix = toVariableUniformSuffix(variable.getKey());
            if (!suffix.isEmpty()) {
                values.put(suffix, parseVariableValue(variable.getValue()));
            }
        }
        return new VariableSnapshot(Map.copyOf(values), variables.size());
    }

    void setVariableProvider(@NotNull Supplier<? extends Map<String, String>> variableProvider) {
    ...
    record FrameSnapshot(float time, float timeDelta, float frameRate, @NotNull float[] iMouse, @NotNull float[] date, float guiScale, @NotNull float[] mouse, @NotNull float[] mouseDelta, @NotNull int[] mouseButtons, @NotNull int[] mouseClickCount, @NotNull int[] mouseReleaseCount, @NotNull float[] mouseScroll, @NotNull float[] mouseScrollTotal, @NotNull int[] keyEvent, int keyEventCount, @NotNull int[] charEvent, int charEventCount, @NotNull int[] dateParts, @NotNull int[] timeParts, int dayOfYear, int weekOfYear, int unixTimeSeconds, int unixTimeMilliseconds, float partialTick, float gameDeltaTicks, float realtimeDeltaTicks, int inWorld, int paused, float opacity, int screenWidth, int screenHeight, int areaWidth, int areaHeight, @NotNull Map<String, VariableValue> variables, int originalVariableCount) {
        ...
        void write(@NotNull GlslStd140Layout layout, @NotNull ByteBuffer buffer, @NotNull PassContext pass, @NotNull ChannelResolution[] channels) {
            ...
            layout.writeFloats(buffer, "konkreteAreaOffset", pass.areaXPx(), pass.areaYPxBottom());
            layout.writeFloats(buffer, "konkreteAreaSize", this.areaWidth, this.areaHeight);
            layout.writeFloats(buffer, "konkreteAreaPosition", pass.areaXPx(), pass.areaYPxBottom());
            layout.writeFloats(buffer, "konkreteAreaTopLeft", pass.areaXPx(), pass.areaYPxTop());
            layout.writeFloats(buffer, "konkreteScreenSize", this.screenWidth, this.screenHeight);
            layout.writeFloats(buffer, "konkreteGuiScale", this.guiScale);
            layout.writeFloats(buffer, "konkreteMouse", this.mouse);
            layout.writeFloats(buffer, "konkreteMouseDelta", this.mouseDelta);
            layout.writeInts(buffer, "konkreteMouseButtons", this.mouseButtons);
            layout.writeInts(buffer, "konkreteMouseClickCount", this.mouseClickCount);
            layout.writeInts(buffer, "konkreteMouseReleaseCount", this.mouseReleaseCount);
            layout.writeFloats(buffer, "konkreteMouseScroll", this.mouseScroll);
            layout.writeFloats(buffer, "konkreteMouseScrollTotal", this.mouseScrollTotal);
            layout.writeInts(buffer, "konkreteKeyEvent", this.keyEvent);
            layout.writeInts(buffer, "konkreteKeyEventCount", this.keyEventCount);
            layout.writeInts(buffer, "konkreteCharEvent", this.charEvent);
            layout.writeInts(buffer, "konkreteCharEventCount", this.charEventCount);
            layout.writeInts(buffer, "konkreteDateParts", this.dateParts);
            layout.writeInts(buffer, "konkreteTimeParts", this.timeParts);
            layout.writeInts(buffer, "konkreteDayOfYear", this.dayOfYear);
            layout.writeInts(buffer, "konkreteWeekOfYear", this.weekOfYear);
            layout.writeInts(buffer, "konkreteUnixTimeSeconds", this.unixTimeSeconds);
            layout.writeInts(buffer, "konkreteUnixTimeMilliseconds", this.unixTimeMilliseconds);
            layout.writeFloats(buffer, "konkretePartialTick", this.partialTick);
            layout.writeFloats(buffer, "konkreteGameDeltaTicks", this.gameDeltaTicks);
            layout.writeFloats(buffer, "konkreteRealtimeDeltaTicks", this.realtimeDeltaTicks);
            layout.writeInts(buffer, "konkreteInWorld", this.inWorld);
            layout.writeInts(buffer, "konkreteIsPaused", this.paused);
            layout.writeFloats(buffer, "konkreteOpacity", this.opacity);
            layout.writeInts(buffer, "konkreteVariableCount", this.originalVariableCount);
        ...
        private void writeVariables(@NotNull GlslStd140Layout layout, @NotNull ByteBuffer buffer) {
            for (Map.Entry<String, VariableValue> entry : this.variables.entrySet()) {
                ...
                writeFloatIfPresent(layout, buffer, "konkreteVarFloat_" + suffix, value.floatValue());
                writeIntIfPresent(layout, buffer, "konkreteVarInt_" + suffix, value.intValue());
                writeIntIfPresent(layout, buffer, "konkreteVarBool_" + suffix, value.boolValue());
                writeFloatsIfPresent(layout, buffer, "konkreteVarVec2_" + suffix, value.vector()[0], value.vector()[1]);
                writeFloatsIfPresent(layout, buffer, "konkreteVarVec3_" + suffix, value.vector()[0], value.vector()[1], value.vector()[2]);
                writeFloatsIfPresent(layout, buffer, "konkreteVarVec4_" + suffix, value.vector());
                writeIntIfPresent(layout, buffer, "konkreteVarExists_" + suffix, 1);
            }
        }
