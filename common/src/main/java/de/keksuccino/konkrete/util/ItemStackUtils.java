...
/** Utility methods for item stack. */
...
public final class ItemStackUtils {
    ...
    public static ItemStack createDisplayStack(Item item, int count) {
        ...
        DataComponentPatch fallbackComponents = DataComponentPatch.builder().set(DataComponents.ITEM_NAME, net.minecraft.network.chat.Component.translatable(item.getDescriptionId())).set(DataComponents.ITEM_MODEL, BuiltInRegistries.ITEM.getKey(item)).set(DataComponents.MAX_STACK_SIZE, 64).build();
        return new ItemStack(Holder.direct(item, getAvailableComponents(item)), count, fallbackComponents);
    }

    /** Creates the gui item stack. */
