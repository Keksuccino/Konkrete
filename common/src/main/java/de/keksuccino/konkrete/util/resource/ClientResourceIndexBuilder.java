...
final class ClientResourceIndexBuilder {
    ...
    static Set<Identifier> build(@NotNull ResourceManager resourceManager) {
        ...
        try (Stream<PackResources> packStream = resourceManager.listPacks()) {
            ...
            LOGGER.error("[KONKRETE] Failed to obtain the active client resource-pack list for the resource picker!", ex);
            return Set.of();
        }
    ...
    private static void applyFilter(@NotNull PackResources pack, @NotNull Set<Identifier> locations) {
        ...
        try {
            ...
            LOGGER.error("[KONKRETE] Failed to read client resource filters from pack '{}'; continuing without its filter.", getPackName(pack), ex);
            return;
        }
        ...
        try {
            ...
            LOGGER.error("[KONKRETE] Failed to apply client resource filters from pack '{}'; continuing with the entries filtered so far.", getPackName(pack), ex);
        }
    }
    ...
    private static void collectPackLocations(@NotNull PackResources pack, @NotNull Set<Identifier> locations) {
        ...
        try {
            ...
            LOGGER.error("[KONKRETE] Failed to obtain client resource namespaces from pack '{}'; skipping its resources.", getPackName(pack), ex);
            return;
        }
        ...
        try {
            ...
            LOGGER.error("[KONKRETE] Failed to snapshot client resource namespaces from pack '{}'; skipping its resources.", getPackName(pack), ex);
            return;
        }

        for (String namespace : namespaces) {
            if (namespace == null || !Identifier.isValidNamespace(namespace)) {
                LOGGER.warn("[KONKRETE] Ignoring invalid client resource namespace '{}' reported by pack '{}'.", namespace, getPackName(pack));
                continue;
            }
            try {
                ...
                LOGGER.error("[KONKRETE] Failed to enumerate client resources in namespace '{}' from pack '{}'; keeping the valid entries reported before the failure.", namespace, getPackName(pack), ex);
            }
        }
    }
    ...
    private static void collectLocation(@NotNull PackResources pack, @NotNull String requestedNamespace, @Nullable Identifier location, @NotNull Set<Identifier> locations) {
        if (location == null) {
            LOGGER.warn("[KONKRETE] Ignoring a null client resource reported by pack '{}' for namespace '{}'.", getPackName(pack), requestedNamespace);
            return;
        }
        if (!requestedNamespace.equals(location.getNamespace())) {
            LOGGER.warn("[KONKRETE] Ignoring client resource '{}' reported by pack '{}' while enumerating namespace '{}'.", location, getPackName(pack), requestedNamespace);
            return;
        }
