...
/** Renders GUI blur into Minecraft's deferred GUI pipeline. */
public final class GuiBlurRenderer {
    ...
    private static final String BLUR_CONFIG_UNIFORM_KONKRETE = "BlurConfig";
    private static final String GUI_BLUR_CONFIG_UNIFORM_KONKRETE = "GuiBlurConfig";
    private static final String SAMPLER_INFO_UNIFORM_KONKRETE = "SamplerInfo";
    private static final String IN_SAMPLER_KONKRETE = "InSampler";
    private static final String ORIGINAL_SAMPLER_KONKRETE = "OriginalSampler";
    private static final String BLUR_SAMPLER_KONKRETE = "BlurSampler";
    private static final int BLUR_CONFIG_UBO_SIZE_KONKRETE = 12;
    private static final int GUI_BLUR_CONFIG_UBO_SIZE_KONKRETE = 80;
    private static final int SINGLE_INPUT_SAMPLER_INFO_UBO_SIZE_KONKRETE = 16;
    private static final int DOUBLE_INPUT_SAMPLER_INFO_UBO_SIZE_KONKRETE = 24;
    private static final float[] BLUR_RADIUS_MULTIPLIERS_KONKRETE = new float[]{1.0F, 1.0F, 0.5F, 0.5F, 0.25F, 0.25F};
    private static final BindGroupLayout BOX_BLUR_BIND_GROUP_LAYOUT_KONKRETE = BindGroupLayout.builder()
            .withSampler(IN_SAMPLER_KONKRETE)
            .withUniform(SAMPLER_INFO_UNIFORM_KONKRETE, UniformType.UNIFORM_BUFFER)
            .withUniform(BLUR_CONFIG_UNIFORM_KONKRETE, UniformType.UNIFORM_BUFFER)
            .build();
    private static final BindGroupLayout SCREEN_COPY_BIND_GROUP_LAYOUT_KONKRETE = BindGroupLayout.builder()
            .withSampler(IN_SAMPLER_KONKRETE)
            .withUniform(SAMPLER_INFO_UNIFORM_KONKRETE, UniformType.UNIFORM_BUFFER)
            .build();
    private static final BindGroupLayout GUI_BLUR_BIND_GROUP_LAYOUT_KONKRETE = BindGroupLayout.builder()
            .withSampler(ORIGINAL_SAMPLER_KONKRETE)
            .withSampler(BLUR_SAMPLER_KONKRETE)
            .withUniform(SAMPLER_INFO_UNIFORM_KONKRETE, UniformType.UNIFORM_BUFFER)
            .withUniform(GUI_BLUR_CONFIG_UNIFORM_KONKRETE, UniformType.UNIFORM_BUFFER)
            .build();
    private static final RenderPipeline BOX_BLUR_PIPELINE_KONKRETE = RenderPipeline.builder(RenderPipelines.POST_PROCESSING_SNIPPET)
            .withLocation(Identifier.fromNamespaceAndPath("konkrete", "pipeline/box_blur"))
            .withVertexShader("core/screenquad")
            .withFragmentShader(Identifier.fromNamespaceAndPath("konkrete", "post/box_blur"))
            .withShaderDefine("KONKRETE_MAX_BLUR_RADIUS", GuiBlurRadius.MAX_RADIUS)
            .withBindGroupLayout(BOX_BLUR_BIND_GROUP_LAYOUT_KONKRETE)
            .build();
    private static final RenderPipeline SCREEN_COPY_PIPELINE_KONKRETE = RenderPipeline.builder(RenderPipelines.POST_PROCESSING_SNIPPET)
            .withLocation(Identifier.fromNamespaceAndPath("konkrete", "pipeline/copy_screen"))
            .withVertexShader("core/screenquad")
            .withFragmentShader(Identifier.fromNamespaceAndPath("konkrete", "post/copy_screen"))
            .withBindGroupLayout(SCREEN_COPY_BIND_GROUP_LAYOUT_KONKRETE)
            .build();
    private static final RenderPipeline GUI_BLUR_PIPELINE_KONKRETE = RenderPipeline.builder(RenderPipelines.POST_PROCESSING_SNIPPET)
            .withLocation(Identifier.fromNamespaceAndPath("konkrete", "pipeline/gui_blur"))
            .withVertexShader("core/screenquad")
            .withFragmentShader(Identifier.fromNamespaceAndPath("konkrete", "post/gui_blur"))
            .withBindGroupLayout(GUI_BLUR_BIND_GROUP_LAYOUT_KONKRETE)
            .build();
    private static final GpuBuffer[] BLUR_CONFIG_BUFFERS_KONKRETE = new GpuBuffer[BLUR_RADIUS_MULTIPLIERS_KONKRETE.length];

    private static TextureTarget blurOriginalTarget_Konkrete;
    private static TextureTarget blurSwapTarget_Konkrete;
    private static TextureTarget blurBlurredTarget_Konkrete;
    private static GpuBuffer boxBlurSamplerInfoBuffer_Konkrete;
    private static GpuBuffer guiBlurSamplerInfoBuffer_Konkrete;
    private static GpuBuffer guiBlurConfigBuffer_Konkrete;
    private static int blurTargetWidth_Konkrete;
    private static int blurTargetHeight_Konkrete;
    ...
    /** Renders blur area scaled into the active GUI extraction pass. */
    ...
    /** Renders blur area round top corners scaled into the active GUI extraction pass. */
    ...
    /** Renders blur area round bottom corners scaled into the active GUI extraction pass. */
    ...
    /** Renders blur area round all corners scaled into the active GUI extraction pass. */
    ...
    /** Renders blur area circle scaled into the active GUI extraction pass. */
    ...
    public static float convertFramebufferBlurRadiusToGui(float framebufferBlurRadius) {
        return (float)(framebufferBlurRadius / getEffectiveGuiScale_Konkrete());
    }

    /**
     * Renders a blur area using Konkrete's blur intensity setting (a normalized multiplier, e.g., 0.25–3.0).
    ...
    public static void renderBlurAreaWithIntensity(@Nonnull GuiGraphicsExtractor graphics, float x, float y, float width, float height, float baseBlurRadius, float cornerRadius, @Nonnull DrawableColor tint, float partial) {
        float appliedRadius = Math.max(0.0F, baseBlurRadius * UIConfiguration.get().blurIntensity());
        renderBlurAreaInternal(graphics, x, y, width, height, appliedRadius, CornerRadii.uniform(cornerRadius), tint, partial);
    }

    /** Renders blur area with intensity scaled into the active GUI extraction pass. */
    ...
    /**
     * Blur area using Konkrete's blur intensity with only the top corners rounded.
     */
    ...
    /** Renders blur area with intensity round top corners scaled into the active GUI extraction pass. */
    ...
    /**
     * Blur area using Konkrete's blur intensity with only the bottom corners rounded.
     */
    ...
    /** Renders blur area with intensity round bottom corners scaled into the active GUI extraction pass. */
    ...
    /**
     * Blur area using Konkrete's blur intensity with individually specified corner radii (top-left, top-right, bottom-right, bottom-left).
     */
    ...
    /** Renders blur area with intensity round all corners scaled into the active GUI extraction pass. */
    ...
    private static void _renderBlurArea(GuiGraphicsExtractor graphics, float partial, BlurArea area) {
        ...
        GuiRenderState renderState = ((AccessorMixinGuiGraphicsExtractor) graphics).get_guiRenderState_Konkrete();
    ...
    /** Executes queued blur area. */
    public static void executeQueuedBlurArea_Konkrete(@Nonnull QueuedBlurArea queuedBlurArea) {
        ...
        BlurArea area = queuedBlurArea.area_Konkrete;
        double guiScale = getEffectiveGuiScale_Konkrete();
        ...
        RenderRotationUtil.Rotation2D maskRotation = queuedBlurArea.maskRotation_Konkrete;
        ...
        ensureBlurTargets_Konkrete(targetWidth, targetHeight);
        updateSamplerInfoUniforms_Konkrete(targetWidth, targetHeight);
        updateBlurConfigUniforms_Konkrete(scaledX, scaledY, scaledWidth, scaledHeight, blurRadius, scaledRadii, area.shapeType, area.roundness, maskRotation, tint);
        executeBlurPasses_Konkrete(mainTarget, scissor);
    }

    /** Closes . */
    public static void close_Konkrete() {
        closeBlurTargets_Konkrete();
        closeBuffer_Konkrete(boxBlurSamplerInfoBuffer_Konkrete);
        closeBuffer_Konkrete(guiBlurSamplerInfoBuffer_Konkrete);
        closeBuffer_Konkrete(guiBlurConfigBuffer_Konkrete);
        boxBlurSamplerInfoBuffer_Konkrete = null;
        guiBlurSamplerInfoBuffer_Konkrete = null;
        guiBlurConfigBuffer_Konkrete = null;
        for (int i = 0; i < BLUR_CONFIG_BUFFERS_KONKRETE.length; i++) {
            closeBuffer_Konkrete(BLUR_CONFIG_BUFFERS_KONKRETE[i]);
            BLUR_CONFIG_BUFFERS_KONKRETE[i] = null;
        }
    }

    private static void ensureBlurTargets_Konkrete(int width, int height) {
        if (blurOriginalTarget_Konkrete != null && blurTargetWidth_Konkrete == width && blurTargetHeight_Konkrete == height) {
            return;
        }
        closeBlurTargets_Konkrete();
        blurOriginalTarget_Konkrete = new TextureTarget("Konkrete GUI blur original", width, height, false, GpuFormat.RGBA8_UNORM);
        blurSwapTarget_Konkrete = new TextureTarget("Konkrete GUI blur swap", width, height, false, GpuFormat.RGBA8_UNORM);
        blurBlurredTarget_Konkrete = new TextureTarget("Konkrete GUI blur blurred", width, height, false, GpuFormat.RGBA8_UNORM);
        blurTargetWidth_Konkrete = width;
        blurTargetHeight_Konkrete = height;
    }

    private static void closeBlurTargets_Konkrete() {
        if (blurOriginalTarget_Konkrete != null) {
            blurOriginalTarget_Konkrete.destroyBuffers();
            blurOriginalTarget_Konkrete = null;
        }
        if (blurSwapTarget_Konkrete != null) {
            blurSwapTarget_Konkrete.destroyBuffers();
            blurSwapTarget_Konkrete = null;
        }
        if (blurBlurredTarget_Konkrete != null) {
            blurBlurredTarget_Konkrete.destroyBuffers();
            blurBlurredTarget_Konkrete = null;
        }
        blurTargetWidth_Konkrete = 0;
        blurTargetHeight_Konkrete = 0;
    }

    private static void updateSamplerInfoUniforms_Konkrete(int width, int height) {
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ByteBuffer data = Std140Builder.onStack(stack, SINGLE_INPUT_SAMPLER_INFO_UBO_SIZE_KONKRETE)
                    .putVec2(width, height)
                    .putVec2(width, height)
                    .get();
            boxBlurSamplerInfoBuffer_Konkrete = updateUniformBuffer_Konkrete(boxBlurSamplerInfoBuffer_Konkrete, "box blur sampler info", data);
        }
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ByteBuffer data = Std140Builder.onStack(stack, DOUBLE_INPUT_SAMPLER_INFO_UBO_SIZE_KONKRETE)
                    .putVec2(width, height)
                    .putVec2(width, height)
                    .putVec2(width, height)
                    .get();
            guiBlurSamplerInfoBuffer_Konkrete = updateUniformBuffer_Konkrete(guiBlurSamplerInfoBuffer_Konkrete, "GUI blur sampler info", data);
        }
    }

    private static void updateBlurConfigUniforms_Konkrete(float x, float y, float width, float height, float blurRadius, CornerRadii cornerRadii, float shapeType, float roundness, RenderRotationUtil.Rotation2D rotation, DrawableColor.FloatColor tint) {
        for (int i = 0; i < BLUR_RADIUS_MULTIPLIERS_KONKRETE.length; i++) {
            float directionX = (i & 1) == 0 ? 1.0F : 0.0F;
            float directionY = (i & 1) == 0 ? 0.0F : 1.0F;
            try (MemoryStack stack = MemoryStack.stackPush()) {
                ByteBuffer data = Std140Builder.onStack(stack, BLUR_CONFIG_UBO_SIZE_KONKRETE)
                        .putVec2(directionX, directionY)
                        .putFloat(blurRadius * BLUR_RADIUS_MULTIPLIERS_KONKRETE[i])
                        .get();
                BLUR_CONFIG_BUFFERS_KONKRETE[i] = updateUniformBuffer_Konkrete(BLUR_CONFIG_BUFFERS_KONKRETE[i], "box blur config " + i, data);
            }
        }

        try (MemoryStack stack = MemoryStack.stackPush()) {
            ByteBuffer data = Std140Builder.onStack(stack, GUI_BLUR_CONFIG_UBO_SIZE_KONKRETE)
                    ...
                    .get();
            guiBlurConfigBuffer_Konkrete = updateUniformBuffer_Konkrete(guiBlurConfigBuffer_Konkrete, "GUI blur config", data);
        }
    }

    private static GpuBuffer updateUniformBuffer_Konkrete(@Nullable GpuBuffer currentBuffer, String uniformName, ByteBuffer data) {
        if (currentBuffer == null || currentBuffer.isClosed() || currentBuffer.size() != data.remaining() || (currentBuffer.usage() & GpuBuffer.USAGE_COPY_DST) == 0) {
            GpuBuffer newBuffer = RenderSystem.getDevice().createBuffer(() -> "Konkrete GUI blur " + uniformName, GpuBuffer.USAGE_UNIFORM | GpuBuffer.USAGE_COPY_DST, data);
            closeBuffer_Konkrete(currentBuffer);
            return newBuffer;
        }
    ...
    private static void executeBlurPasses_Konkrete(RenderTarget mainTarget, BlurScissor scissor) {
        if (mainTarget.getColorTexture() == null || blurOriginalTarget_Konkrete == null || blurSwapTarget_Konkrete == null || blurBlurredTarget_Konkrete == null || blurOriginalTarget_Konkrete.getColorTexture() == null) {
            return;
        }
        if (boxBlurSamplerInfoBuffer_Konkrete == null || guiBlurSamplerInfoBuffer_Konkrete == null || guiBlurConfigBuffer_Konkrete == null) {
            return;
        }
        for (GpuBuffer blurConfigBuffer : BLUR_CONFIG_BUFFERS_KONKRETE) {
        ...
        if (RenderingUtils.isVulkanActive()) {
            executeTextureCopy_Konkrete(commandEncoder, mainTarget, blurOriginalTarget_Konkrete, scissor);
        } else {
            GpuSampler copySampler = RenderSystem.getSamplerCache().getClampToEdge(FilterMode.NEAREST);
            executeScreenCopyPass_Konkrete(commandEncoder, mainTarget, blurOriginalTarget_Konkrete, scissor, copySampler);
        }
        executeBoxBlurPass_Konkrete(commandEncoder, "Konkrete GUI blur horizontal 0", blurOriginalTarget_Konkrete, blurSwapTarget_Konkrete, BLUR_CONFIG_BUFFERS_KONKRETE[0], scissor, blurSampler);
        executeBoxBlurPass_Konkrete(commandEncoder, "Konkrete GUI blur vertical 0", blurSwapTarget_Konkrete, blurBlurredTarget_Konkrete, BLUR_CONFIG_BUFFERS_KONKRETE[1], scissor, blurSampler);
        executeBoxBlurPass_Konkrete(commandEncoder, "Konkrete GUI blur horizontal 1", blurBlurredTarget_Konkrete, blurSwapTarget_Konkrete, BLUR_CONFIG_BUFFERS_KONKRETE[2], scissor, blurSampler);
        executeBoxBlurPass_Konkrete(commandEncoder, "Konkrete GUI blur vertical 1", blurSwapTarget_Konkrete, blurBlurredTarget_Konkrete, BLUR_CONFIG_BUFFERS_KONKRETE[3], scissor, blurSampler);
        executeBoxBlurPass_Konkrete(commandEncoder, "Konkrete GUI blur horizontal 2", blurBlurredTarget_Konkrete, blurSwapTarget_Konkrete, BLUR_CONFIG_BUFFERS_KONKRETE[4], scissor, blurSampler);
        executeBoxBlurPass_Konkrete(commandEncoder, "Konkrete GUI blur vertical 2", blurSwapTarget_Konkrete, blurBlurredTarget_Konkrete, BLUR_CONFIG_BUFFERS_KONKRETE[5], scissor, blurSampler);
        executeGuiBlurPass_Konkrete(commandEncoder, mainTarget, scissor, blurSampler);
    }

    private static void executeTextureCopy_Konkrete(CommandEncoder commandEncoder, RenderTarget inputTarget, RenderTarget outputTarget, BlurScissor scissor) {
    ...
    private static void executeScreenCopyPass_Konkrete(CommandEncoder commandEncoder, RenderTarget inputTarget, RenderTarget outputTarget, BlurScissor scissor, GpuSampler sampler) {
        ...
        try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "Konkrete GUI blur source copy", outputTarget.getColorTextureView(), Optional.empty())) {
            renderPass.setPipeline(SCREEN_COPY_PIPELINE_KONKRETE);
            RenderSystem.bindDefaultUniforms(renderPass);
            renderPass.setUniform(SAMPLER_INFO_UNIFORM_KONKRETE, boxBlurSamplerInfoBuffer_Konkrete);
            renderPass.bindTexture(IN_SAMPLER_KONKRETE, inputTarget.getColorTextureView(), sampler);
            enableScissor_Konkrete(renderPass, scissor);
            renderPass.draw(3, 1, 0, 0);
            renderPass.disableScissor();
        }
    }

    private static void executeBoxBlurPass_Konkrete(CommandEncoder commandEncoder, String label, RenderTarget inputTarget, RenderTarget outputTarget, GpuBuffer blurConfigBuffer, BlurScissor scissor, GpuSampler sampler) {
        if (inputTarget.getColorTextureView() == null || outputTarget.getColorTextureView() == null) {
            return;
        }

        try (RenderPass renderPass = commandEncoder.createRenderPass(() -> label, outputTarget.getColorTextureView(), Optional.empty())) {
            renderPass.setPipeline(BOX_BLUR_PIPELINE_KONKRETE);
            RenderSystem.bindDefaultUniforms(renderPass);
            renderPass.setUniform(SAMPLER_INFO_UNIFORM_KONKRETE, boxBlurSamplerInfoBuffer_Konkrete);
            renderPass.setUniform(BLUR_CONFIG_UNIFORM_KONKRETE, blurConfigBuffer);
            renderPass.bindTexture(IN_SAMPLER_KONKRETE, inputTarget.getColorTextureView(), sampler);
            enableScissor_Konkrete(renderPass, scissor);
            renderPass.draw(3, 1, 0, 0);
            renderPass.disableScissor();
    ...
    private static void executeGuiBlurPass_Konkrete(CommandEncoder commandEncoder, RenderTarget mainTarget, BlurScissor scissor, GpuSampler sampler) {
        if (mainTarget.getColorTextureView() == null || blurOriginalTarget_Konkrete.getColorTextureView() == null || blurBlurredTarget_Konkrete.getColorTextureView() == null) {
            return;
        }

        try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "Konkrete GUI blur composite", mainTarget.getColorTextureView(), Optional.empty(), mainTarget.useDepth ? mainTarget.getDepthTextureView() : null, OptionalDouble.empty())) {
            renderPass.setPipeline(GUI_BLUR_PIPELINE_KONKRETE);
            RenderSystem.bindDefaultUniforms(renderPass);
            renderPass.setUniform(SAMPLER_INFO_UNIFORM_KONKRETE, guiBlurSamplerInfoBuffer_Konkrete);
            renderPass.setUniform(GUI_BLUR_CONFIG_UNIFORM_KONKRETE, guiBlurConfigBuffer_Konkrete);
            renderPass.bindTexture(ORIGINAL_SAMPLER_KONKRETE, blurOriginalTarget_Konkrete.getColorTextureView(), sampler);
            renderPass.bindTexture(BLUR_SAMPLER_KONKRETE, blurBlurredTarget_Konkrete.getColorTextureView(), sampler);
            enableScissor_Konkrete(renderPass, scissor);
            renderPass.draw(3, 1, 0, 0);
            renderPass.disableScissor();
        }
    ...
    private static void enableScissor_Konkrete(RenderPass renderPass, BlurScissor scissor) {
        renderPass.enableScissor(scissor.x(), scissor.y(), scissor.width(), scissor.height());
    }

    private static void closeBuffer_Konkrete(@Nullable GpuBuffer buffer) {
    ...
    public static final class QueuedBlurArea {
        ...
        private final BlurArea area_Konkrete;
        private final RenderRotationUtil.Rotation2D maskRotation_Konkrete;

        private QueuedBlurArea(BlurArea area, RenderRotationUtil.Rotation2D maskRotation) {
            this.area_Konkrete = area;
            this.maskRotation_Konkrete = maskRotation;
        }

    }

    /** Captures the immutable state needed for GUI blur render. */
    public record GuiBlurRenderState(QueuedBlurArea queuedBlurArea, ScreenRectangle bounds) implements GuiRenderPhaseAction {

        /** Executes this state's deferred render commands. */
        @Override
        public void executeRender_Konkrete() {
            executeQueuedBlurArea_Konkrete(this.queuedBlurArea);
        }
    ...
    /**
     * Blur areas are extracted in the same logical coordinate space as Konkrete's precise GUI projection.
     ...
     */
    private static double getEffectiveGuiScale_Konkrete() {
