...
/** Provides {@code AfmaBinaryFrameIndexHelper} transformations and validation used by the AFMA codec. */
public final class AfmaBinaryFrameIndexHelper {

    /** Archive entry path for frame index data. */
    public static final String FRAME_INDEX_ENTRY_PATH = "frame_index.bin";
    /** Serialized magic value identifying frame index data. */
    public static final int FRAME_INDEX_MAGIC = 0x41464958; // AFIX
    /** Serialized version value for frame index data. */
    public static final int FRAME_INDEX_VERSION = 3;
    /** Bit flag indicating multi copy shared width in serialized data. */
    protected static final int MULTI_COPY_SHARED_WIDTH_FLAG = 1;
    /** Bit flag indicating multi copy shared height in serialized data. */
    protected static final int MULTI_COPY_SHARED_HEIGHT_FLAG = 1 << 1;

    private AfmaBinaryFrameIndexHelper() {
    }

    /** Encodes the frame index for the AFMA codec. */
    @NotNull
    ...
    /** Decodes the frame index for the AFMA codec. */
    @NotNull
    ...
    /** Writes the sequence to the AFMA codec output. */
    ...
    /** Reads the sequence from the AFMA codec input. */
    protected static @NotNull List<AfmaFrameDescriptor> readSequence(@NotNull DataInputStream in) throws IOException {
        int count = readVarInt(in);
        if (count < 0 || count > ResourceRuntime.getSafetyLimits().maxFrameCount()) {
            throw new IOException("AFMA binary frame sequence count exceeds resource safety limits");
        }
    ...
    /** Writes the descriptor to the AFMA codec output. */
    ...
    /** Reads the descriptor from the AFMA codec input. */
    ...
    /** Writes the copy rect to the AFMA codec output. */
    ...
    /** Reads the copy rect from the AFMA codec input. */
    ...
    /** Writes the multi copy to the AFMA codec output. */
    ...
    /** Reads the multi copy from the AFMA codec input. */
    protected static @NotNull AfmaMultiCopy readMultiCopy(@NotNull DataInputStream in) throws IOException {
        ...
        if (rectCount < 2 || rectCount > ResourceRuntime.getSafetyLimits().maxArchiveEntries()) {
            throw new IOException("AFMA multi-copy rectangle count exceeds resource safety limits");
        }
    ...
    /** Writes the patch bounds to the AFMA codec output. */
    ...
    /** Writes the patch region to the AFMA codec output. */
    ...
    /** Writes the payload id to the AFMA codec output. */
    ...
    /** Reads the payload path from the AFMA codec input. */
    ...
    /** Maps the AFMA codec operation type to its binary identifier. */
    ...
    /** Resolves the id to an AFMA codec operation type. */
    ...
    /** Writes the signed var int to the AFMA codec output. */
    ...
    /** Reads the signed var int from the AFMA codec input. */
