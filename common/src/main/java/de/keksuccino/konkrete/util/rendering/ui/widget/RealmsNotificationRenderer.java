...
import de.keksuccino.konkrete.mixin.mixins.common.client.AccessorMixinRealmsNotificationsScreen;
...
public class RealmsNotificationRenderer {
    ...
    private final AccessorMixinRealmsNotificationsScreen screenAccess;
    private final Minecraft minecraft;
    private final int screenWidth;
    private final int screenHeight;
    private BooleanSupplier previewModeSupplier = () -> false;
    ...
    public RealmsNotificationRenderer(@NotNull RealmsNotificationsScreen screen, int screenWidth, int screenHeight) {
        this.screenAccess = (AccessorMixinRealmsNotificationsScreen)screen;
    ...
    public void renderIcons(GuiGraphicsExtractor guiGraphics, int color) {
        ...
        boolean isValidClient = this.screenAccess.get_validClient_Konkrete().getNow(false);
        if (isValidClient) {
    ...
    public int getNumberOfPendingInvites() {
        ...
        return this.screenAccess.get_numberOfPendingInvites_Konkrete();
    }
    ...
    public boolean hasUnseenNotifications() {
        if (isPreviewMode()) return true;
        return AccessorMixinRealmsNotificationsScreen.get_hasUnseenNotifications_Konkrete();
    }
    ...
    public boolean hasUnreadNews() {
        if (isPreviewMode()) return true;
        return AccessorMixinRealmsNotificationsScreen.get_hasUnreadNews_Konkrete();
    }
    ...
    public boolean isTrialAvailable() {
        if (isPreviewMode()) return true;
        return AccessorMixinRealmsNotificationsScreen.get_trialAvailable_Konkrete();
    }

    /**
     * Supplies whether synthetic notification values should be rendered for previews.
     *
     * @param previewModeSupplier preview-state supplier
     * @return this renderer
     */
    public RealmsNotificationRenderer setPreviewModeSupplier(@NotNull BooleanSupplier previewModeSupplier) {
        this.previewModeSupplier = Objects.requireNonNull(previewModeSupplier, "previewModeSupplier");
        return this;
    }

    /**
     * Returns whether preview values should replace live Realms state.
     *
     * @return whether preview mode is active
     */
    protected boolean isPreviewMode() {
        return this.previewModeSupplier.getAsBoolean();
    }
