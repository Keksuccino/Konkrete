...
import de.keksuccino.konkrete.mixin.mixins.common.client.AccessorMixinSplashRenderer;
...
public class MinecraftSplashRenderer {
    ...
    public static final MinecraftSplashRenderer DEFAULT_INSTANCE = new MinecraftSplashRenderer(((AccessorMixinSplashRenderer)Objects.requireNonNullElse(Minecraft.getInstance().gui.splashManager().getSplash(), new SplashRenderer(Component.translatable("ERROR")))).get_splash_Konkrete());

    /** Width offset in GUI pixels. */
    public static final int WIDTH_OFFSET = 123;
    /** Height offset in GUI pixels. */
    public static final int HEIGHT_OFFSET = 69;

    private final Component splash;

    /** Uses the supplied component as the Minecraft-logo splash text. */
    public MinecraftSplashRenderer(Component splash) {
        this.splash = splash;
    }

    /** Returns default position x. */
    public int getDefaultPositionX(int screenWidth) {
        return (screenWidth / 2) + WIDTH_OFFSET;
    }

    /** Returns default position y. */
