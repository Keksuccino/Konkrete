...
public abstract class ResourceHandler<R extends Resource, F extends FileType<R>> {
    ...
    /** Holds live resources by canonical source or caller-provided registration key. */
    protected final Map<String, R> resources = new ConcurrentHashMap<>();
    /** Holds canonical sources whose codec construction previously failed. */
    ...
    public R get(@NotNull ResourceSource resourceSource) {
        ...
        try {
            ...
            if (fileType == null) {
                LOGGER.error("[KONKRETE] Failed to register resource! Unsupported file type or failed to identify file type: " + resourceSource + " (RESOURCE HANDLER: " + this.getClass() + ")");
                this.addToFailedSources(resourceSource);
                return null;
            }
            if (resourceSource.getSourceType() == ResourceSourceType.WEB) {
                if (!fileType.isWebAllowed()) {
                    LOGGER.error("[KONKRETE] Failed to register web resource! File type does not support web sources: " + fileType + " (Source: " + resourceSource + ")" + " (RESOURCE HANDLER: " + this.getClass() + ")");
                    this.addToFailedSources(resourceSource);
                    return null;
                }
                ...
                if (!fileType.isLocationAllowed()) {
                    LOGGER.error("[KONKRETE] Failed to register location resource! File type does not support location sources: " + fileType + " (Source: " + resourceSource + ")" + " (RESOURCE HANDLER: " + this.getClass() + ")");
                    this.addToFailedSources(resourceSource);
                    return null;
                }
                Identifier loc = Identifier.tryParse(resourceSource.getSourceWithoutPrefix());
                if (loc == null) {
                    LOGGER.error("[KONKRETE] Failed to register location resource! Unable to parse Identifier: " + resourceSource + " (RESOURCE HANDLER: " + this.getClass() + ")");
                    this.addToFailedSources(resourceSource);
                    return null;
                }
                ...
                if (!fileType.isLocalAllowed()) {
                    LOGGER.error("[KONKRETE] Failed to register local resource! File type does not support local sources: " + fileType + " (Source: " + resourceSource + ")" + " (RESOURCE HANDLER: " + this.getClass() + ")");
                    this.addToFailedSources(resourceSource);
                    return null;
                }
            ...
            LOGGER.error("[KONKRETE] Failed to register resource: " + resourceSource + " (RESOURCE HANDLER: " + this.getClass() + ")", ex);
            this.addToFailedSources(resourceSource);
        }
        return null;
    }
    ...
    /**
     * Transfers the resource to this handler when the exact caller-provided key is absent.
     * A collision with a different instance, or registration during shutdown, rejects and closes the supplied resource; registering the already-owned instance is idempotent.
     */
    public void registerIfKeyAbsent(@NotNull String key, @NotNull R resource) {
        ...
        synchronized (this.lifecycleLock) {
            ...
            if (!rejectResource) {
                R existingResource = this.getResourceMap().putIfAbsent(key, resource);
                if (existingResource == null) {
                    LOGGER.debug("[KONKRETE] Registering resource with key: " + key + " (RESOURCE HANDLER: " + this.getClass() + ")");
                } else if (existingResource != resource) {
                    rejectResource = true;
                }
            }
        }
    ...
    /** Returns whether a live mapping currently exists for the exact cache key. */
    public boolean hasResource(@NotNull String key) {
        if (this.shuttingDown) return false;
        return this.getResourceMap().containsKey(Objects.requireNonNull(key));
    }

    /** Gets a live cached resource, detaching and closing a stale mapping without removing a concurrent replacement. */
    @Nullable
    ...
    /** Publishes a newly constructed resource or closes it when another owner won the registration race. */
    @Nullable
        ...
        if (resource != null) {
            ...
            synchronized (this.lifecycleLock) {
                ...
                if (!rejectResource) {
                    ...
                    else LOGGER.debug("[KONKRETE] Registering resource with source: " + resourceSource + " (RESOURCE HANDLER: " + this.getClass() + ")");
                }
            }
            if (rejectResource) {
            ...
            if (this.addToFailedSources(resourceSource)) {
                LOGGER.error("[KONKRETE] Failed to register resource! Resource was NULL: " + resourceSource + " (RESOURCE HANDLER: " + this.getClass() + ")");
            }
        }
        return null;
    }

    /** Records a canonical source failure unless shutdown already owns the handler. */
    ...
    /** Returns the live concurrent resource map; subclasses must use the lifecycle lock indirectly through handler operations for compound mutations. */
    @NotNull
    protected Map<String, R> getResourceMap() {
        return this.resources;
    }

    /** Returns the live concurrent set of canonical sources whose construction failed. */
    @NotNull
    protected Set<String> getFailedSourcesSet() {
        return this.failedSources;
    }

    /** Returns the file types this handler can decode, in matching priority order. */
    @NotNull
    ...
    public void release(@NotNull String key, boolean isKeyResourceSource) {
        ...
        if (isKeyResourceSource) {
            key = ResourceSource.of(key).getSourceWithPrefix();
            this.getFailedSourcesSet().remove(key);
        }
        R resource;
        synchronized (this.lifecycleLock) {
