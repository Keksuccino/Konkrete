...
/** Implements the interactive window body for number picker. */
public class NumberPickerWindowBody<N extends Number> extends PiPWindowBody implements InitialWidgetFocusScreen {
    ...
    public interface ValueAdapter<N extends Number> {
        ...
        /** Formats a nullable numeric value for the editor. */
        @NotNull
        String formatValue(@Nullable N value);

        /** Maps a value into the slider's numeric coordinate space. */
        double toSliderValue(@NotNull N value);

        /** Converts a slider coordinate back into the represented number type. */
        @NotNull
        N fromSliderValue(double sliderValue);

        /** Returns whether labels should omit fractional digits. */
    ...
    /** Initializes a numeric picker with range, preset, conversion strategy, and lifecycle callbacks. */
    ...
    /** Initializes resources required by this value adapter. */
    @Override
    protected void init() {
        int inputHeight = 20;
        int sliderHeight = 20;
        ...
        this.cancelButton = this.addRenderableWidget(new ExtendedButton((this.width / 2) - 5 - 100, buttonY, 100, buttonHeight, Component.translatable("konkrete.common_components.cancel"), button -> {
        ...
        this.doneButton = this.addRenderableWidget(new ExtendedButton((this.width / 2) + 5, buttonY, 100, buttonHeight, Component.translatable("konkrete.common_components.done"), button -> {
    ...
    /** Handles window closed externally for this value adapter. */
    @Override
