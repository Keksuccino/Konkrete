...
import de.keksuccino.konkrete.util.input.MouseUtils;
...
/** Normalizes selected vanilla input events. */
public class VanillaEvents {

    private static final Logger LOGGER = LogManager.getLogger();

    /** Modifier mask used when vanilla supplies no keyboard modifiers. */
    ...
    /** Returns the latest vanilla mouse button info or dummy. */
    @NotNull
    public static MouseButtonInfo getLatestVanillaMouseButtonInfoOrDummy() {
        ...
        if ((latestVanillaMouseButtonInfoCachingTime != -1L) && ((latestVanillaMouseButtonInfoCachingTime + 2000) < now)) {
            LOGGER.warn("[KONKRETE] Getting an possibly outdated latestVanillaMouseButtonInfo! Last caching time is more than 2 seconds in the past! This should be avoided.", new Exception("Outdated MouseButtonInfo in VanillaEvents!"));
        }
        if (latestVanillaMouseButtonInfo != null) return latestVanillaMouseButtonInfo;
        LOGGER.warn("[KONKRETE] Getting latestVanillaMouseButtonInfo before caching it! This should be avoided.", new Exception("No MouseButtonInfo cached in VanillaEvents!"));
        return new MouseButtonInfo(0, -1);
    }

    /** Updates latest vanilla mouse button info. */
