...
public final class WatermediaDeferredPlayerReleaseTracker {
    ...
    /** Transfers release ownership for a live reflected player unless shutdown has terminally sealed registration. */
    ...
    /** Removes and releases a tracked player exactly once on its normal render-thread release path. */
    ...
    /** Terminally seals registration and releases every remaining player under the shutdown backend policy. */
    public static void shutdown() {
        ...
        for (Object player : playersToRelease) {
            try {
                ...
                LOGGER.error("[KONKRETE] Failed to release a deferred Watermedia player during client shutdown!", throwable);
            }
        }
    }
