 ...
 * without loading the full Konkrete runtime.</p>
 */
public final class AfmaDiffStandaloneTool {
    ...
    /** Runs the standalone AFMA creator command. */
    ...
    /** Prints the usage for the AFMA creator command. */
    ...
    /** Compares the sequence data in the AFMA creator. */
    @NotNull
    ...
    /** Updates the this decode frame state in the AFMA creator. */
    ...
    /** Applies the residual payload to the AFMA creator. */
    ...
    /** Applies the sparse payload to the AFMA creator. */
    ...
    /** Returns the frame offset produced by the AFMA creator. */
    protected static int frameOffset(int x, int y, int width) {
        return (y * width) + x;
    }

    /** Converts the abgr from the AFMA creator representation. */
    ...
    /** Converts the argb from the AFMA creator representation. */
    ...
    /** Measures the frame diff for the AFMA creator. */
    @NotNull
    ...
    /** Inserts the worst frame into the ordered AFMA creator state. */
    ...
    /** Prints the summary for the AFMA creator command. */
    ...
    /** Reports {@code FrameDiffResult} measurements produced by the AFMA creator. */
    ...
    /** Models {@code SequenceDiffSummary} state used by the AFMA creator. */
    ...
    /** Models {@code Arguments} state used by the AFMA creator. */
    protected record Arguments(@Nullable File afmaFile,
        ...
        /** Parses resource state from the AFMA creator input. */
        @NotNull
        ...
        /** Returns the required file produced by the AFMA creator. */
        @NotNull
        ...
        /** Returns the optional file produced by the AFMA creator. */
        @Nullable
    ...
    /** Reads {@code ArchiveReader} input for the AFMA creator. */
    protected interface ArchiveReader extends AutoCloseable {

        /** Opens the reader matching the archive container version. */
        @NotNull
        ...
        /** Returns the decoded archive metadata. */
        @NotNull
        AfmaMetadata metadata();

        /** Returns the decoded archive frame index. */
        @NotNull
        AfmaFrameIndex frameIndex();

        /** Reads one archive payload into memory. */
        @NotNull
        byte[] readPayloadBytes(@NotNull String path) throws Exception;
    }

    /** Reads {@code ContainerArchiveReader} input for the AFMA creator. */
    protected static final class ContainerArchiveReader implements ArchiveReader {
        ...
        /** Initializes a new {@code ContainerArchiveReader} for AFMA creator use. */
        ...
        /** Returns the metadata used by the AFMA creator. */
        @Override
        public @NotNull AfmaMetadata metadata() {
            return this.metadata;
        }

        /** Returns the frame index produced by the AFMA creator. */
        @Override
        public @NotNull AfmaFrameIndex frameIndex() {
            return this.frameIndex;
        }

        /** Reads the payload bytes from the AFMA creator input. */
        @Override
        ...
        /** Loads the chunk for the AFMA creator. */
        @NotNull
        ...
        /** Closes the random-access AFMA v2 archive; cached chunk arrays remain eligible for collection. */
        @Override
    ...
    /** Reads {@code ZipArchiveReader} input for the AFMA creator. */
    protected static final class ZipArchiveReader implements ArchiveReader {
        ...
        /** Initializes a new {@code ZipArchiveReader} for AFMA creator use. */
        ...
        /** Closes the legacy AFMA ZIP archive. */
        @Override
        public void close() throws Exception {
            this.zipFile.close();
        }
    ...
    /** Normalizes the payload path for the AFMA creator. */
    @NotNull
    ...
    /** Reads the zip entry from the AFMA creator input. */
    @NotNull
    ...
    /** Reads the fully from the AFMA creator input. */
    @NotNull
    ...
    /** Inflates the raw chunk within configured AFMA creator byte limits. */
    @NotNull
    ...
    /** Parses the metadata from the AFMA creator input. */
    @NotNull
