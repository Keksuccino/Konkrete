...
/** Stores one editable line and its cached formatting state. */
public class TextEditorLine extends ExtendedEditBox {

    /** Editor that owns this line and coordinates document-level selection. */
    public TextEditorWindowBody parent;
    /** Previous value used to detect edits during the next tick. */
    protected String lastTickValue = "";
    /** Whether pointer dragging currently extends the text selection. */
    public boolean isInMouseHighlightingMode = false;
    /** Legacy handle-self flag retained for source compatibility; current input routing does not consult it. */
    protected final boolean handleSelf2;
    /** Width in GUI units for text. */
    public int textWidth = 0;
    /** Zero-based document index of this line. */
    public int lineIndex = 0;
    /** Cached GUI-space x coordinate of the selection start. */
    protected int currentHighlightPosXStart = 0;
    /** Cached GUI-space x coordinate of the selection end. */
    protected int currentHighlightPosXEnd = 0;
    /** Line-local character index during the current formatting pass. */
    protected int currentCharacterRenderIndex = 0;

    /** Whether a horizontal arrow key was held during the previous input update. */
    protected static boolean leftRightArrowWasDown = false;

    /** Positions one editable line with its font, filter, and owning editor. */
    public TextEditorLine(Font font, int x, int y, int width, int height, boolean handleSelf, @Nullable CharacterFilter characterFilter, TextEditorWindowBody parent) {
        ...
        if (characterFilter != null) {
            this.setCharacterFilter(new de.keksuccino.konkrete.util.input.CharacterFilter() {
                /** Returns whether allowed char. */
                @Override
                ...
                /** Removes characters rejected by this editor's character filter. */
                @Override
    ...
    public void extractWidgetRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partial) {
        ...
        if (this.isVisible()) {
            ...
            int textColorInt = this.getAsAccessor().get_isEditable_Konkrete() ? this.getAsAccessor().get_textColor_Konkrete() : this.getAsAccessor().get_textColorUneditable_Konkrete();
            int cursorPos = this.getCursorPosition() - this.getAsAccessor().get_displayPos_Konkrete();
            int highlightPos = this.getAsAccessor().get_highlightPos_Konkrete() - this.getAsAccessor().get_displayPos_Konkrete();
            String text = this.getValue();
            boolean isCursorInsideVisibleText = cursorPos >= 0 && cursorPos <= text.length();
            boolean renderCursor = this.isFocused() && (Util.getMillis() - this.getAsAccessor().get_focusedTime_Konkrete()) / 300L % 2L == 0L && isCursorInsideVisibleText;
            float textHeight = UIBase.getUITextHeightNormal();
            float textX = this.getAsAccessor().get_bordered_Konkrete() ? this.getX() + 4.0F : this.getX() + 1.0F;
            float textY = this.getAsAccessor().get_bordered_Konkrete()
            ...
            boolean isCursorNotAtEndOfLine = this.getCursorPosition() < this.getValue().length() || this.getValue().length() >= this.getAsAccessor().get_maxLength_Konkrete();
            ...
            if (this.getAsAccessor().get_hint_Konkrete() != null && text.isEmpty() && !this.isFocused()) {
                UIBase.renderText(graphics, this.getAsAccessor().get_hint_Konkrete(), textXAfterCursor, textY, textColorInt);
            }

            if (!isCursorNotAtEndOfLine && this.getAsAccessor().get_suggestion_Konkrete() != null) {
                UIBase.renderText(graphics, this.getAsAccessor().get_suggestion_Konkrete(), cursorPosRender - 1, textY, -8355712);
            }
            ...
            if (highlightPos != cursorPos) {
                ...
                graphics.textHighlight(this.currentHighlightPosXStart, (int) (textY - 1), this.currentHighlightPosXEnd, (int) (textY + 1 + textHeight), this.getAsAccessor().get_invertHighlightedTextColor_Konkrete());
            } else {
    ...
    /** Returns whether in editor area. */
    ...
    /** Returns whether highlighted hovered. */
    ...
    public int getActualHeight() {
        int h = this.height;
        if (this.getAsAccessor().get_bordered_Konkrete()) {
            h += 2;
        }
        return h;
    }
    ...
    /** Returns as accessor. */
    public AccessorMixinEditBox getAsAccessor() {
        return (AccessorMixinEditBox) this;
    }

    /** Sets cursor position for this text editor line. */
    @Override
    ...
    /** Sets value for this text editor line. */
    @Override
    ...
    /** Sets max length for this text editor line. */
    @Override
