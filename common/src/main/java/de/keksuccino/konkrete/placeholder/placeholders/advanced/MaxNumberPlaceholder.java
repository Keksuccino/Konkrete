...
/** Applies the max number transformation for {@code maxnum}. */
public class MaxNumberPlaceholder extends Placeholder {

    private static final Logger LOGGER = LogManager.getLogger();

    /** Creates the {@code maxnum} placeholder. */
    ...
    public String getReplacementFor(DeserializedPlaceholderString dps) {
        ...
        if ((firstNumber != null) && (secondNumber != null)) {
            try {
                ...
                LOGGER.error("[KONKRETE] Failed to evaluate max-number placeholder: {}", dps.placeholderString, ex);
                return null;
            }
        }
        LOGGER.error("[KONKRETE] Failed to parse 'Max Number' placeholder: " + dps.placeholderString);
        return null;
    }
    ...
    public @NotNull String getDisplayName() {
        return I18n.get("konkrete.placeholders.max_number");
    }

    @Override
    public List<String> getDescription() {
        return Arrays.asList(LocalizationUtils.splitLocalizedStringLines("konkrete.placeholders.max_number.desc"));
    }
