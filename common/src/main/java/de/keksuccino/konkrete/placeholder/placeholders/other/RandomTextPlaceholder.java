...
import de.keksuccino.konkrete.util.threading.AsyncRefreshingValueCache;
...
/** Selects text from inline, local, or web sources with interval and bounded content caches. */
public class RandomTextPlaceholder extends Placeholder {
    ...
    private static final AsyncRefreshingValueCache<String, List<String>> CONTENT_CACHE = createContentCache(task -> Thread.ofVirtual().name("Konkrete-RandomTextLoader").start(task), RandomTextPlaceholder::loadSource, System::currentTimeMillis);

    private static final BoundedConcurrentCache<String, RandomTextPackage> RANDOM_TEXT_INTERVALS = new BoundedConcurrentCache<>(512, 1_048_576L, (source, ignored) -> 128L + source.length() * 2L);

    private final AsyncRefreshingValueCache<String, List<String>> contentCache;

    /** Creates the {@code randomtext} placeholder. */
    ...
    public String getReplacementFor(DeserializedPlaceholderString dps) {
        ...
        RandomTextPackage p = RANDOM_TEXT_INTERVALS.compute(sourceString, (ignored, current) -> current != null ? current : new RandomTextPackage());
    ...
    private static AsyncRefreshingValueCache<String, List<String>> createContentCache(@NotNull AsyncRefreshingValueCache.TaskLauncher taskLauncher, @NotNull AsyncRefreshingValueCache.ValueLoader<String, List<String>> sourceLoader, @NotNull LongSupplier timeSource) {
        return new AsyncRefreshingValueCache<>(taskLauncher, source -> List.copyOf(sourceLoader.load(source)), (source, exception) -> {
            LOGGER.error("[KONKRETE] Failed to read source for RandomTextPlaceholder: " + source, exception);
            return List.of();
        }, (source, exception) -> LOGGER.error("[KONKRETE] Failed to start the asynchronous Random Text source read: " + source, exception), timeSource);
    }
    ...
    private static List<String> loadFromUrl(String url) {
        ...
        try (InputStream stream = WebUtils.openResourceStream(url, WebUtils.WebResourceType.TEXT)) {
            if (stream == null) {
                LOGGER.warn("[KONKRETE] Failed to open URL stream: " + url);
                return lines;
            }
            ...
            LOGGER.error("[KONKRETE] Error reading from URL: " + url, e);
        }
    ...
    private static List<String> loadFromFile(String pathString) {
        try {
            ...
            if (!path.isFile() || !path.getPath().toLowerCase(Locale.ROOT).endsWith(".txt")) {
                LOGGER.warn("[KONKRETE] File not found or not a .txt file: " + pathString);
                return new ArrayList<>();
            }
            ...
            LOGGER.error("[KONKRETE] Error reading file: " + pathString, e);
            return new ArrayList<>();
        }
    }
    ...
    static boolean isPlainText(String path) {
        ...
        boolean looksLikeFilePath = path.trim().toLowerCase(Locale.ROOT).endsWith(".txt");
    ...
    public @NotNull String getDisplayName() {
        return I18n.get("konkrete.placeholders.randomtext");
    }

    @Override
    public List<String> getDescription() {
        return Arrays.asList(LocalizationUtils.splitLocalizedStringLines("konkrete.placeholders.randomtext.desc"));
    }
    ...
    public @NotNull DeserializedPlaceholderString getDefaultPlaceholderString() {
        ...
        values.put("source", "/config/example.txt");
