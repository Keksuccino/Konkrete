...
import de.keksuccino.konkrete.util.threading.AsyncLoadCoordinator;
...
final class JsonPlaceholderWebCache {
    ...
    private final BoundedConcurrentCache<String, List<String>> cachedPlaceholders = new BoundedConcurrentCache<>(512, 8_388_608L, JsonPlaceholderWebCache::weighValues);
    private final BoundedConcurrentCache<String, Boolean> invalidSources = new BoundedConcurrentCache<>(512, 1_048_576L, (source, ignored) -> 64L + source.length() * 2L);
    ...
    boolean isInvalidSource(@NotNull String source) {
        return this.invalidSources.get(Objects.requireNonNull(source, "source")) != null;
    }
    ...
    List<String> cleanupTimedOut() {
        ...
        for (String placeholder : timedOut) LOGGER.warn("[KONKRETE] Placeholder update timed out for: {}", placeholder);
        return timedOut;
    }
    ...
    private Lookup getExisting(@NotNull String placeholder, @NotNull String source) {
        if (this.invalidSources.get(source) != null) return Lookup.invalid();
    ...
    private void launch(@NotNull AsyncLoadCoordinator.Claim<String> claim, @NotNull String source, @NotNull String jsonPath) {
        try {
            ...
            LOGGER.error("[KONKRETE] Failed to start the web JSON loader for '{}'.", source, exception);
        } catch (Error error) {
            this.loadCoordinator.abandon(claim);
            throw error;
        }
    ...
    private void loadAndPublish(@NotNull AsyncLoadCoordinator.Claim<String> claim, @NotNull String source, @NotNull String jsonPath) {
        ...
        try {
            ...
            if (!claim.isCancelled()) LOGGER.error("[KONKRETE] Error while caching a web JSON in the JsonPlaceholder!", exception);
            return;
        }
        this.loadCoordinator.publishIfCurrent(claim, () -> {
            if (result.valid()) {
                ...
                this.invalidSources.put(source, Boolean.TRUE);
            }
        });
    }

    private static long weighValues(@NotNull String placeholder, @NotNull List<String> values) {
        long weight = 64L + placeholder.length() * 2L;
        for (String value : values) {
            long valueWeight = 32L + value.length() * 2L;
            if (valueWeight > Long.MAX_VALUE - weight) return Long.MAX_VALUE;
            weight += valueWeight;
        }
        return weight;
    }
