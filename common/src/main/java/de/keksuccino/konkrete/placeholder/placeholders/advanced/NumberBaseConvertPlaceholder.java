...
/** Applies the number base convert transformation for {@code number_base_convert}. */
public class NumberBaseConvertPlaceholder extends Placeholder {
    ...
    /** Creates the {@code number_base_convert} placeholder. */
    ...
    public String getReplacementFor(DeserializedPlaceholderString dps) {
        ...
        if ((inputValue == null) || inputValue.trim().isEmpty()) {
            LOGGER.error("[KONKRETE] Missing 'input' value for 'Number Base Converter' placeholder: {}", dps.placeholderString);
            return null;
        }
        ...
        try {
            ...
            LOGGER.error("[KONKRETE] Failed to convert number base for placeholder: {}", dps.placeholderString, ex);
            return null;
        }
    }
    ...
    public @NotNull String getDisplayName() {
        return I18n.get("konkrete.placeholders.number_base_convert");
    }

    @Override
    public List<String> getDescription() {
        return Arrays.asList(LocalizationUtils.splitLocalizedStringLines("konkrete.placeholders.number_base_convert.desc"));
    }
    ...
    private int parseBase(@Nullable String value, int fallback, @NotNull String placeholderString, @NotNull String valueName) {
        ...
        if (!MathUtils.isInteger(trimmed)) {
            LOGGER.error("[KONKRETE] Invalid '{}' value for 'Number Base Converter' placeholder: {} (value: {})", valueName, placeholderString, value);
            return fallback;
        }
        try {
            int parsed = Integer.parseInt(trimmed);
            if ((parsed < MIN_BASE) || (parsed > MAX_BASE)) {
                LOGGER.error("[KONKRETE] '{}' value out of range ({}, {}) for 'Number Base Converter' placeholder: {} (value: {})", valueName, MIN_BASE, MAX_BASE, placeholderString, value);
                return fallback;
            }
            return parsed;
        } catch (NumberFormatException ex) {
            LOGGER.error("[KONKRETE] Failed to parse '{}' value for 'Number Base Converter' placeholder: {}", valueName, placeholderString, ex);
            return fallback;
        }
    }
    ...
    private ParsedNumber parseInputNumber(@NotNull String input, int base, @NotNull String placeholderString) {
        ...
        if (working.isEmpty()) {
            LOGGER.error("[KONKRETE] Empty 'input' value for 'Number Base Converter' placeholder: {}", placeholderString);
            return null;
        }
        ...
        if ((separatorIndex != -1) && (working.indexOf(DECIMAL_SEPARATOR, separatorIndex + 1) != -1)) {
            LOGGER.error("[KONKRETE] Invalid 'input' value (multiple decimal separators) for 'Number Base Converter' placeholder: {}", placeholderString);
            return null;
        }
    ...
    private int decodeDigit(char character, int base, @NotNull String placeholderString) {
        ...
        if (digit < 0) {
            LOGGER.error("[KONKRETE] Invalid character '{}' for base {} in 'Number Base Converter' placeholder: {}", character, base, placeholderString);
        }
        return digit;
    }
