	...
	public void prepareAnimation() {
		...
		try {
			...
			if (f.exists() && f.isFile() && Files.getFileExtension(f.getName()).equalsIgnoreCase("gif")) {
				...
				LOGGER.error("[KONKRETE] Unable to load GIF animation! File not found or not a valid GIF: " + this.resourceDir, new Exception());
			}
			this.ready = true;
		} catch (Exception ex) {
			LOGGER.error("[KONKRETE] Error while trying to prepare GIF animation!", ex);
		}
	}
