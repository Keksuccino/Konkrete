package de.keksuccino.persephone.properties;
...
import de.keksuccino.persephone.file.FileUtils;
	...
	public static PropertiesSet getProperties(@NotNull String filePath) {
		...
		if (f.exists() && f.isFile()) {
			...
			for (String s : lines) {
				...
				if (comp.endsWith("{")) {
					if (!insideData) {
						...
						LOGGER.warn("[PERSEPHONE] Invalid properties found in '" + filePath + "'! (Leaking properties section; Missing '}')");
			...
			if (propertiesType != null) {
				...
				LOGGER.warn("[PERSEPHONE] Invalid properties file found: " + filePath + " (Missing properties type)");
			}
		}
		return null;
	}
	...
	public static void writeProperties(@NotNull PropertiesSet propertiesSet, @NotNull String filePath) {
		try {
			...
			if (f.getName().contains(".") && !f.getName().startsWith(".")) {
				...
				LOGGER.error("[PERSEPHONE] Failed to write properties! Given path is not a file: " + filePath, new FileNotFoundException());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
