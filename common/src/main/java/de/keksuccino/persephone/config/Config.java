package de.keksuccino.persephone.config;
...
import de.keksuccino.persephone.config.ConfigEntry.EntryType;
import de.keksuccino.persephone.config.exceptions.InvalidValueException;
import de.keksuccino.persephone.math.MathUtils;
...
public class Config {
	...
	public void syncConfig() {
		...
		if (oldConfig == null) {
			LOGGER.error("[PERSEPHONE] Config backup failed: " + this.path, new NullPointerException("Backup config was NULL!"));
		}
		if (!this.config.exists()) {
			try {
				this.config.createNewFile();
			} catch (IOException ex) {
				LOGGER.error("[PERSEPHONE] Error while trying to create config file!", ex);
			}
		}
		...
		try {
			...
			LOGGER.error("[PERSEPHONE] Error while trying to write to config file or deleting backup config file!", ex);
        }
	...
	private File backupConfig() {
		...
		if (!back.exists()) {
			try {
				...
				LOGGER.error("[PERSEPHONE] Error while trying to create backup config file!", ex);
			}
		}
		boolean b = false;
		for (String s : data) {
		...
		try {
			...
			LOGGER.error("[PERSEPHONE] Error while trying to write to backup config file!", ex);
        }
		IOUtils.closeQuietly(writer);
	...
	private List<String> getTextFileData() {
		...
		if (this.config.exists()) {
			...
			try {
				...
				LOGGER.error("[PERSEPHONE] Error while trying to read data from config file!", ex);
			}
