...
#moj_import <konkrete:konkrete_rounded_box.glsl>
...
void main() {
    ...
    float alpha = konkreteRoundedBoxAlpha(p, halfSize, cornerRadii);
    ...
    if (borderThickness > 0.0) {
        ...
        if (innerHalfSize.x > 0.0 && innerHalfSize.y > 0.0) {
            float innerAlpha = konkreteRoundedBoxAlpha(p, innerHalfSize, innerRadii);
